...
class Pipe:
    ...
    async def run_thinking(
        ...
        prompt += "Do not answer the user directly. Do not write any code. Only produce a hidden reasoning chain, starting by rephrasing the user prompt. \n"
        prompt += "Be concise - there is limited time. Try to keep your chain of thought to about eight paragraphs. \n"
        prompt += f"User Query: {query}"
        logger.info(f"Thinking prompt: {prompt}")
    ...
    async def run_responding(
        ...
        prompt += f"The reasoning within the provided <reasoning></reasoning> tags was generated by yourself in an earlier step and was not entered by the user. The end of the reasoning chain may be incomplete or cut-off due to time constraints. Use it to help you respond in concise and helpful manner to the query: {query}"
    ...
    async def pipe(
        ...
        if (
            ...
            if self.max_thinking_time_reached:
                ...
                await self.set_status_end(
                    f"Thought for {self.total_thinking_tokens} tokens in {total_thought_duration} seconds",
                    __event_emitter__,
                )
            return ""
        else:
